...
public class EvaluerAdherentController {
            ...
            try {
                ...
                for (Node node : vBoxContainer1.getChildren()) {
                    if (node instanceof HBox) {
                        ...
                        // Récupérer l'ID de l'adhérent sélectionné dans la combobox
                                int adherentID = getMembreIDFromNomPrenom( cmbAdh.getValue()); // Remplacer par la logique pour obtenir l'ID de l'adhérent

                                // Insérer une entrée dans la table EvaluationAdherent
                                EvaluationAdherent evaluationAdherent = new EvaluationAdherent(Integer.parseInt(valeur),Metrique.getMetriqueIDParNom(connection, metriqueNom),adherentID);
                                evaluationAdherent.insererEvaluationAdherent(connection);
                        }
                    }
                }
...
private void clearFields() {
    cmbAdh.getSelectionModel().clearSelection();
